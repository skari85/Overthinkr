...
export interface Achievement {
  ...
  thresholdType: "conversations" | "overthinking" | "valid" | "what-if-scenarios"
...
const defaultAchievements: Achievement[] = [
  ...
  },
  {
    id: "deep-thinker",
    name: "Deep Thinker",
    description: "Explore 5 'What If' scenarios.",
    icon: "Lightbulb",
    thresholdType: "what-if-scenarios",
    thresholdValue: 5,
    unlocked: false,
  },
  {
    ...
    description: "Explore 20 'What If' scenarios.",
    icon: "FlaskConical",
    thresholdType: "what-if-scenarios",
    thresholdValue: 20,
    unlocked: false,
  },
  {
    ...
    description: "Receive 10 'Overthinking' and 10 'Valid Concern' classifications.",
    icon: "Scale",
    thresholdType: "conversations", // This will be checked manually in checkAndUnlockAchievements
    thresholdValue: 20, // Combined threshold
    unlocked: false,
  },
]
...
export function checkAndUnlockAchievements(): Achievement[] {
  ...
  const updatedAchievements = currentAchievements.map((ach) => {
    ...
    if (ach.thresholdType === "conversations" && metrics.totalConversations >= ach.thresholdValue) {
      ...
      meetsThreshold = true
    } else if (ach.thresholdType === "what-if-scenarios" && metrics.totalWhatIfSessions >= ach.thresholdValue) {
      meetsThreshold = true
    } else if (ach.id === "balanced-perspective" && metrics.overthinkingCount >= 10 && metrics.validCount >= 10) {
      meetsThreshold = true
    }
